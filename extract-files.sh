...
function blob_fixup() {
    case "${1}" in
        vendor/etc/camera/ingres_enhance_motiontuning.xml|vendor/etc/camera/ingres_motiontuning.xml)
            [ "$2" = "" ] && return 0
        ...
        vendor/lib64/libcamximageformatutils.so)
            ...
            ;;
        vendor/lib64/hw/audio.primary.taro-ingres.so)
            [ "$2" = "" ] && return 0
            "${PATCHELF_0_17_2}" --set-soname "audio.primary.taro-ingres.so" "${2}"
            ;;
